...
const TopStories = () => {
    ...
    useEffect (
         ()=> {
        axios.get("https://api.nytimes.com/svc/topstories/v2/arts.json?api-key=xjqswikF0mXhASZuIhlJYIJ9k2zqVi7W") 
        .then(
            (json_data)=> {
    ...
    return (
        <div>
             <h1 className = "h1_nav"> 
                <span>Dileep'S API TESTS </span>   
                <span> 
                <Link to="/" className = "go_to_link"> Go To Movies API </Link>
                </span> 
            </h1>
            <div className = "title_div"> <h1>Top Stories</h1>  </div>
          <h1>
              {
                  TopStories.map (
                      (each_topstory)=> {
                          return (
                              <div key={each_topstory.title}>
